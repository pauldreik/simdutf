...
template<UtfEncodings encoding>
struct ValidationFunctionTrait
{};

template<>
struct ValidationFunctionTrait<UtfEncodings::UTF16BE>
{
    static inline auto Validation = &simdutf::implementation::validate_utf16be;
    static inline auto ValidationWithErrors = &simdutf::implementation::validate_utf16be_with_errors;
    static inline std::string ValidationWithErrorsName{
        NAMEOF(&simdutf::implementation::validate_utf16be_with_errors)};
    static inline std::string ValidationName{NAMEOF(&simdutf::implementation::validate_utf16be)};
    using RawType = char16_t;
};
template<>
struct ValidationFunctionTrait<UtfEncodings::UTF16LE>
{
    static inline auto Validation = &simdutf::implementation::validate_utf16le;
    static inline auto ValidationWithErrors = &simdutf::implementation::validate_utf16le_with_errors;
    static inline std::string ValidationWithErrorsName{
        NAMEOF(&simdutf::implementation::validate_utf16le_with_errors)};
    static inline std::string ValidationName{NAMEOF(&simdutf::implementation::validate_utf16le)};
    using RawType = char16_t;
};
template<>
struct ValidationFunctionTrait<UtfEncodings::UTF32>
{
    static inline auto Validation = &simdutf::implementation::validate_utf32;
    static inline auto ValidationWithErrors = &simdutf::implementation::validate_utf32_with_errors;
    static inline std::string ValidationWithErrorsName{
        NAMEOF(&simdutf::implementation::validate_utf32_with_errors)};
    static inline std::string ValidationName{NAMEOF(&simdutf::implementation::validate_utf32)};
    using RawType = char32_t;
};
template<>
struct ValidationFunctionTrait<UtfEncodings::UTF8>
{
    static inline auto Validation = &simdutf::implementation::validate_utf8;
    static inline auto ValidationWithErrors = &simdutf::implementation::validate_utf8_with_errors;
    static inline std::string ValidationWithErrorsName{
        NAMEOF(&simdutf::implementation::validate_utf8_with_errors)};
    static inline std::string ValidationName{NAMEOF(&simdutf::implementation::validate_utf8)};
    using RawType = char;
};
template<>
struct ValidationFunctionTrait<UtfEncodings::LATIN1>
{
...
struct Conversion
{
    ...
    void fuzz(std::span<const char> chardata) const
    {
        ...
        do {
            ...
            const auto [written, outputs_agree] = do_conversion(from, output_length, inputisvalid);
    ...
    validation_result verify_valid_input(FromSpan src) const
    {
        ...
        for (auto impl : implementations) {
            results.push_back(std::invoke(input_validation, impl, src.data(), src.size()));

            // make sure the validation variant that returns a bool agrees
            const bool validation1 = results.back().error == simdutf::SUCCESS;
            const bool validation2 = std::invoke(ValidationFunctionTrait<From>::Validation,
                                                 impl,
                                                 src.data(),
                                                 src.size());
            if (validation1 != validation2) {
                std::cerr << "begin errormessage for verify_valid_input()\n";
                std::cerr << ValidationFunctionTrait<From>::ValidationWithErrorsName << " gives "
                          << validation1 << " while "
        ...
        if (std::ranges::adjacent_find(results, neq) != results.end()) {
            ...
            std::cerr << "in fuzz case for "
                      << ValidationFunctionTrait<From>::ValidationWithErrorsName << " invoked with "
                      << src.size() << " elements:\n";
    ...
    {
        // all latin1 input is valid. there is no simdutf validation function for it.
    ...
    // length of it.
    template<typename Dummy = void>
        requires std::is_invocable_v<LengthFunction,
                                     const simdutf::implementation *,
                                     const FromType *,
                                     std::size_t>
    std::size_t invoke_lengthcalc(const simdutf::implementation *impl, FromSpan src) const
    {
                                    ...
                                    const bool inputisvalid) const
    {
        ...
        for (std::size_t i = 0; i < implementations.size(); ++i) {
            ...
            if constexpr (use_canary_in_output) {
                ...
                const auto implret2 = std::invoke(conversion,
                                                  impl,
                                                  src.data(),
                                                  src.size(),
                                                  outputbuffer.data());
    ...
    void dump_testcase(FromSpan typedspan, std::ostream &os) const
    {
        ...
        if constexpr (From != UtfEncodings::LATIN1) {
            os << "const auto validation1=implementation."
               << ValidationFunctionTrait<From>::ValidationWithErrorsName << "((const "
        ...
        if (std::is_invocable_v<LengthFunction,
                                const simdutf::implementation *,
                                const FromType *,
                                std::size_t>) {
            os << "const auto outlen=implementation." << lengthcalcname << "((const "
               << nameoftype(FromType{}) << "*) data,\n data_len);\n";
        } else if (std::is_invocable_v<LengthFunction, const simdutf::implementation *, std::size_t>) {
            os << "const auto outlen=implementation." << lengthcalcname << "(data_len);\n";
        } else {
            // programming error
            std::abort();
        }
...
#define ADD(lenfunc, conversionfunc) \
    Conversion<ENCODING_FROM_CONVERSION_NAME(&I::conversionfunc), \
               ...
               decltype(&I::lenfunc), \
               decltype(&I::conversionfunc)> \
    { \
        &I::lenfunc, &I::conversionfunc, std::string{NAMEOF(&I::lenfunc)}, std::string \
        { \
...
#define IGNORE(len, x) \
    Conversion<ENCODING_FROM_CONVERSION_NAME(&I::x), \
               ENCODING_TO_CONVERSION_NAME(&I::x), \
               decltype(&I::len), \
               decltype(&I::x)> \
    { \
...
template<std::size_t... Indices,
         ...
         member_function_pointer... LengthFunction,
         member_function_pointer... ConversionFunction>
void pickfromtupleimpl(
    std::size_t i,
    std::index_sequence<Indices...>,
    const std::tuple<Conversion<From, To, LengthFunction, ConversionFunction>...> &t,
