...
{
    // FIXME - is validate_ascii correct here?
    static inline auto ValidationWithErrors = &simdutf::implementation::validate_ascii_with_errors;
    static inline std::string ValidationWithErrorsName{
        NAMEOF(&simdutf::implementation::validate_ascii_with_errors)};
    using RawType = char;
};
...
#define ADD(len, x) \
    Conversion<ENCODING_FROM_CONVERSION_NAME(&I::x), \
               ...
               decltype(&I::x)> \
    { \
        &I::len, &I::x, std::string{NAMEOF(&I::len)}, std::string \
        { \
            NAMEOF(&I::x) \
        } \
    }
