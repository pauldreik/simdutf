...
inline std::span<const simdutf::implementation *const> get_supported_implementations()
{
...
struct FNV1A_hash
{
    ...
    static constexpr std::uint64_t fnv1ahash_impl(std::span<const unsigned char> bytes)
    {
        auto hash = offset;
    ...
    {
        return fnv1ahash_impl(
            {reinterpret_cast<const unsigned char *>(data.data()), data.size_bytes()});
    }
    ...
    template<typename... Data>
    static std::string as_str(const Data &...data)
    {
...
static_assert(FNV1A_hash::fnv1ahash_impl(std::string_view{""}) == 0xcbf29ce484222325);
static_assert(FNV1A_hash::fnv1ahash_impl(std::string_view{"xyz"}) == 0xbff4aa198026f420);
static_assert(FNV1A_hash::fnv1ahash_impl(std::string{"xyz"}) == 0xbff4aa198026f420);
static_assert(FNV1A_hash::fnv1ahash_impl(std::string_view{"\xFF"}) == 0xaf64724c8602eb6e);
static_assert(FNV1A_hash::fnv1ahash_impl(std::string_view{"\x01\x01\x01\x01"})
              == 0xb5d0e0774c7d7499);
static_assert(FNV1A_hash::fnv1ahash_impl(std::array<unsigned char, 4>{0x01, 0x01, 0x01, 0x01})
              == 0xb5d0e0774c7d7499);
